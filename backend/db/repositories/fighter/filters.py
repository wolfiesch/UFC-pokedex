"""Search and pagination helpers for fighter roster queries."""
...
class FighterSearchFilters:
    """Normalized roster search filters shared across repository layers."""
...
def _validate_streak_type(value: str | None) -> StreakType | None:
    """Return a canonical streak literal when ``value`` is supported."""

    if value is None:
        return None
    candidate = value.strip().lower()
    if candidate not in {"win", "loss", "draw", "none"}:
...
def paginate_roster_entries(
    roster: Sequence[RosterEntry] | Iterable[RosterEntry],
    *,
    limit: int | None,
    offset: int | None,
) -> list[RosterEntry]:
