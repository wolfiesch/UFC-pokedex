"""Helpers for enriching fighters with ranking metadata.

This module centralizes the SQL needed to compute both current and peak
rankings for roster entries. By moving the enrichment workflow outside of the
main repository class we can reuse the logic across services while keeping the
repository focused strictly on data-access concerns.
"""
...
async def fetch_ranking_summaries(
    ...
    *,
    ...
    """Return current and peak ranking summaries for the requested fighters.

    The helper deduplicates incoming identifiers, limits the ranking records to
    the configured ``ranking_source``, and then issues two focused queries to
    capture both the latest rank and the career-best peak for each fighter.
    """
