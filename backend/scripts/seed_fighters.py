#!/usr/bin/env python
"""Seed minimal fighter data into the PostgreSQL database from a JSONL file.

This script loads the minimal fighter columns required by the ``/fighters``
endpoint. The workflow now targets PostgreSQL exclusively because the wider
application stack no longer supports SQLite. Keeping the seeding code aligned
with production infrastructure prevents silent drift between environments and
ensures developers exercise the same database contract locally.
...
POSTGRESQL_IDENTIFIER: Final[str] = "postgresql"


def ensure_postgresql_backend(*, db_type: str | None = None) -> None:
    """Validate that the seeding workflow runs against PostgreSQL only.

    Args:
        db_type: Optional override that primarily exists for unit tests. When
            omitted the helper consults :func:`get_database_type` to avoid
            duplication of connection logic.

    Raises:
        RuntimeError: Raised when the resolved database backend does not match
            the PostgreSQL identifier required by the application.
    """

    resolved_type = (db_type or get_database_type()).lower()
    if resolved_type != POSTGRESQL_IDENTIFIER:
        raise RuntimeError(
            "Fighter seeding requires a PostgreSQL database."
            " Set DATABASE_URL to a PostgreSQL connection string before rerunning the command."
        )
...
async def main() -> int:
    ...
    # Ensure we only run against PostgreSQL. The helper provides an early and
    # explicit failure mode so developers immediately know to configure the
    # required backend before attempting to seed data.
    ensure_postgresql_backend(db_type=db_type)
