#!/usr/bin/env python
"""Scrape fighter images from DuckDuckGo using Playwright MCP."""
...
def search_duckduckgo_images(fighter_name: str, nickname: str | None = None) -> list[dict]:
    """
    Search DuckDuckGo Images for fighter photos.

    Returns list of image results with URL, thumbnail, source, etc.
    Uses DuckDuckGo's JSON API (no Playwright needed for search).
    """
    # Build search query
    query = f"{fighter_name} UFC fighter"
    if nickname:
        query = f'{fighter_name} "{nickname}" UFC fighter'

    # DuckDuckGo's image search API endpoint
...
def select_best_image(images: list[dict]) -> dict | None:
    ...
    PREFERRED_SOURCES = ["espn", "ufc", "sherdog", "wikipedia", "wikimedia"]

    for source in PREFERRED_SOURCES:
        for img in quality_images:
            if source in img.get("source", "").lower() or source in img.get("url", "").lower():
                return img

    # Return first quality image
...
async def scrape_fighter_image(
    ...
    """
    Scrape image for a single fighter.
    ...
    for attempt_num, (name, nick) in enumerate(search_attempts):
        # Search for images
        images = search_duckduckgo_images(name, nick)
        ...
        for retry in range(retry_count):
            ...
            if success:
                ...
                if db_updated:
                    return (fighter_id, True, f"Downloaded from {best_image.get('source', 'DuckDuckGo')}")
                else:
...
async def main(batch_size: int = 50, test_mode: bool = False):
    ...
    console.print("[bold cyan]DuckDuckGo Fighter Image Scraper[/bold cyan]\n")
    ...
    checkpoint_file = Path("data/checkpoints/duckduckgo_scraper.json")
    ...
    # Save final results
    results_file = Path("data/logs/duckduckgo_scraper_results.json")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Scrape fighter images from DuckDuckGo")
