...
class FightMatrixDivisionMapperSpider(scrapy.Spider):
    ...
    def start_requests(self) -> Generator[scrapy.Request, None, None]:
        ...
        yield scrapy.Request(
            url=self.BASE_URL, callback=self.parse_division_dropdown, errback=self.handle_error
        )

    def parse_division_dropdown(
        ...
        division_select = (
            response.css("select")[1]
            if len(response.css("select")) > 1
            else response.css("select")[0]
        )
        ...
        for division_code, division_name in enumerate(divisions_to_test, start=1):
            ...
            yield scrapy.Request(
                ...
                cb_kwargs={"division_name": division_name, "division_code": division_code},
                errback=self.handle_error,
                dont_filter=True,  # Allow multiple requests to same domain
            )

    def verify_division_code(
        self, response: scrapy.http.Response, division_name: str, division_code: int
    ) -> dict:
        """
        ...
        if has_data:
            self.logger.info(
                f"✓ {division_name} → Code {division_code} ({len(fighter_rows)} fighters found)"
            )
        else:
            self.logger.warning(
                f"✗ {division_name} → Code {division_code} (no data found - might be invalid)"
            )
