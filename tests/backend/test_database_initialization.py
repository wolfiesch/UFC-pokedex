...
class TestDatabaseURL:
    ...
    def test_get_database_url_missing_env_raises_runtime_error(
        self, monkeypatch
    ) -> None:
        """Test that a missing DATABASE_URL raises an informative RuntimeError."""
    ...
    def test_get_database_url_rejects_non_postgres_schemes(self, monkeypatch) -> None:
        """Test that non-PostgreSQL URLs are rejected with clear guidance."""
...
class TestPreflightLogging:
    ...
    def test_sanitize_database_url_hides_password(self, mock_get_url, mock_get_type):
        ...
        test_cases = [
            ...
            (
                "postgresql+psycopg://user:pa%25ss@localhost:5432/db",
...
class TestDatabaseInitialization:
    ...
    @patch("backend.main.get_database_url")
    @patch("backend.main.get_database_type")
    async def test_lifespan_rejects_non_postgresql_database(
        self, mock_get_type, mock_get_url
    ) -> None:
        """Test that the lifespan hook refuses to start when PostgreSQL is not configured."""
    ...
    async def test_postgresql_lifespan_warmup(
        self,
        mock_get_engine,
        mock_get_type,
        mock_warmup,
        ...
        """Test that PostgreSQL mode warms connections without calling create_all()."""
